...
times = [dt.datetime(2022,11,20,11,0)]
titles=['IOP3'] # Title for each subplot. The time for the subplot is automatically added after this title.
num_col = 1 # Number of columns for subplot cells
num_rows = 1 # Number of rows for subplot cells
...
radar_fields = ['reflectivity'] # List of fields for each pane
sweep=[0,0,0,0,0,0,0,0,0] # List of sweeps for each pane
    ...
    if radar_on == True:
        ...
        if glob.glob(grid_file) != []:
            ...
            for j in range(len(radar_id)):
                ...
                gate_filter = pyart.correct.GateFilter(radars[j])
                gate_filter.exclude_below('reflectivity', gate_filter_num)
                gate_filter_tuple.append(gate_filter)
...
def plot_rap_data(time, ax):
    if rap_on == True:
        ...
        ax.barbs(data["lon_u"][::5, ::5], data["lat_u"][::5, ::5],
                 data["U85"][::5, ::5], data["V85"][::5, ::5],
                 transform=ccrs.PlateCarree(),
...
def get_cached_rap_data(time):
    ...
    with pygrib.open(filename[0]) as grbs:
        t_grb = grbs.select(name='Temperature', level=850)[0]
        T85 = t_grb.values - 273.15
        lat_t, lon_t = t_grb.latlons()
        t_mask = (lat_t >= lat_min) & (lat_t <= lat_max) & (lon_t >= lon_min) & (lon_t <= lon_max)
        T85 = np.where(t_mask, T85, np.nan)
    
        u_grb = grbs.select(name='U component of wind', level=850)[0]
        # i.select()[192]
        U85 = u_grb.values
        lat_u, lon_u = u_grb.latlons()
        u_mask = (lat_u >= lat_min) & (lat_u <= lat_max) & (lon_u >= lon_min) & (lon_u <= lon_max)
        U85 = np.where(u_mask, U85, np.nan)

        v_grb = grbs.select(name='V component of wind', level=850)[0]
        V85 = v_grb.values
        V85 = np.where(u_mask, V85, np.nan)
        # grb = i.select()[193]
...
plt.savefig(save_dir + 'size_option_3.png', dpi=500, facecolor='white')
plt.show()
# -
